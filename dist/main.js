...
function sliderInit(element) {
  var container = $('<div/>').addClass('slides-container');
  ...
  element.html(container).addClass('slider').append(navigation).attr('currentImage', '0').attr('id', 'slider' + $('.slider').length);
  autoPlay(element);
}
...
function slide(slider) {
  ...
  if (currentImage >= container.children('.slide').length) {
    ...
    var clone = container.children('.slide:first-child').clone(); // ajouter le clone au bandeau

    container.append(clone); // écouter la fin de la transition

    container.on('transitionend', function () {
      // Supprimer l'ecouteur pour qu'il ne joue qu'une fois 
      $(this).off('transitionend'); // enlever la transition sur le bandeau

      container.css('transition', 'none'); // rembobiner le bandeau vers l'image 1

      slider.attr('currentImage', 0);
      slide(slider); // supprimer le clone

      clone.remove(); // remettre la transition sur le bandeau après un timeout
  ...
  if (currentImage <= -1) {
    ...
    var clone = container.children('.slide:last-child').clone(); // ajouter le clone avant la première image
    ...
    container.on('transitionend', function () {
      $(this).off('transitionend'); // enlever la transition

      container.css('transition', 'none'); // rembobiner vers la dernière image

      var lastIndex = container.children('.slide').length - 1 - 1;
      slider.attr('currentImage', lastIndex);
      slide(slider);
      clone.remove(); // remettre la transition
...
function autoPlay(slider) {
  var idSlider = slider.attr('id'); // Suppression de l'intervalle s'il existe

  if (undefined != intervals[idSlider]) {
    var _interval = intervals[idSlider];
